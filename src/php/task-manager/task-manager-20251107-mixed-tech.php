...
class TaskManager {
    ...
    /**
     * Exports all tasks to a JSON file. Returns true on success.
     */
    public function exportTasksToJson(string $filename): bool {
        $serializedTasks = array_map(function(Task $t): array { return $t->toArray(); }, $this->tasks);
        $data = ['tasks' => array_values($serializedTasks), 'nextId' => $this->nextId];
        $written = @file_put_contents($filename, json_encode($data, JSON_PRETTY_PRINT));
        return $written !== false;
    }

    /**
     * Imports tasks from a JSON file and merges them. Returns number of tasks imported.
     */
    public function importTasksFromJson(string $filename): int {
        if (!file_exists($filename)) return 0;
        $data = @file_get_contents($filename);
        if ($data === false) return 0;
        $decoded = json_decode($data, true);
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($decoded) || empty($decoded['tasks'])) return 0;

        $imported = 0;
        foreach ($decoded['tasks'] as $item) {
            try {
                $task = Task::fromArray($item);
                // Avoid ID collision: if exists, assign a new ID
                if (isset($this->tasks[$task->id])) {
                    $newId = $this->generateId();
                    $task = new Task($newId, $task->description, $task->priority, $task->dueDate?->format('Y-m-d'), $task->status, $task->createdAt->format('Y-m-d H:i:s'), $task->completedAt?->format('Y-m-d H:i:s'), $task->tags);
                    $this->tasks[$newId] = $task;
                } else {
                    $this->tasks[$task->id] = $task;
                    // Ensure nextId is always larger than any imported id
        ...
        if ($imported > 0) $this->saveTasks();
        return $imported;
    }

    /**
     * Creates a duplicate of the given task and returns the new task ID, or null on failure.
     */
    public function duplicateTask(int $id): ?int {
        if (!isset($this->tasks[$id])) return null;
        $orig = $this->tasks[$id];
        $newId = $this->generateId();
        $dup = new Task($newId, $orig->description, $orig->priority, $orig->dueDate?->format('Y-m-d'), $orig->status, $orig->createdAt->format('Y-m-d H:i:s'), $orig->completedAt?->format('Y-m-d H:i:s'), $orig->tags);
        $this->tasks[$newId] = $dup;
        $this->saveTasks();
        return $newId;
    }

    /**
     * Bulk update status for multiple task IDs. Returns number of tasks updated.
    ...
    public function bulkUpdateStatus(array $ids, string $newStatus): int {
        ...
        if ($canonicalStatus === null) {
            throw new \InvalidArgumentException("Invalid status provided for bulk update: {$newStatus}. Must be one of: " . implode(', ', self::VALID_STATUSES));
        }
        $count = 0;
        foreach ($ids as $id) {
            $id = (int)$id;
            if (!isset($this->tasks[$id])) continue;
            $task = $this->tasks[$id];
            if ($canonicalStatus === self::STATUS_COMPLETED && !$task->completedAt) {
                $task->completedAt = new \DateTimeImmutable();
            } elseif ($task->completedAt && $canonicalStatus !== self::STATUS_COMPLETED) {
                $task->completedAt = null;
            }
    ...
    public function self_test(): void {
        ...
        // 8. New Feature Tests: Export/Import, Duplicate, Bulk Update
        echo "\n[8] Testing Export/Import, Duplicate, and Bulk Update features:\n";

        $exportFile = 'export_tasks_test.json';
        $totalTests++;
        if ($this->exportTasksToJson($exportFile)) {
            echo "SUCCESS: Exported tasks to $exportFile.\n";
            // Verify file exists and is valid JSON
            $totalTests++;
            if (!file_exists($exportFile)) {
                echo "FAIL: Export file does not exist after export.\n";
                $failedTests += 2;
            } else {
                $passedTests++;
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    echo "FAIL: Exported JSON is invalid.\n";
                    $failedTests += 2;
                } else {
                    echo "SUCCESS: Exported JSON is valid.\n";
                    $passedTests += 2;
                }
            }
        } else {
            echo "FAIL: Export to $exportFile failed.\n";
            $failedTests++;
        }
        
        // Save the count before duplication for import test later
        $taskCountBeforeDuplicate = count($this->tasks);

        // Test duplicate with valid task
        $totalTests++;
        $dupId = $this->duplicateTask($id2);
        if ($dupId !== null && isset($this->tasks[$dupId])) {
            $original = $this->tasks[$id2];
            $duplicate = $this->tasks[$dupId];
            if ($duplicate->description === $original->description && 
                $duplicate->priority === $original->priority &&
                $duplicate->id !== $original->id) {
                echo "SUCCESS: Duplicated Task ID $id2 to new ID $dupId with correct data.\n";
                $passedTests++;
            } else {
                echo "FAIL: Duplicate task has incorrect data.\n";
                $failedTests++;
            }
        } else {
            echo "FAIL: Duplication of Task ID $id2 failed.\n";
            $failedTests++;
        }
        
        // Test duplicate with non-existent task (edge case)
        $totalTests++;
        $invalidDupId = $this->duplicateTask(99999);
        if ($invalidDupId === null) {
            echo "SUCCESS: Duplicate non-existent task correctly returned null.\n";
            $passedTests++;
        } else {
            echo "FAIL: Duplicate non-existent task should return null.\n";
            $failedTests++;
        }

        // Bulk update: mark tasks 1 and 2 as In Progress
        $totalTests++;
        $bulkUpdated = $this->bulkUpdateStatus([$id1, $id2], self::STATUS_IN_PROGRESS);
        if ($bulkUpdated === 2 && 
            $this->tasks[$id1]->status === self::STATUS_IN_PROGRESS && 
            $this->tasks[$id2]->status === self::STATUS_IN_PROGRESS) {
            echo "SUCCESS: Bulk updated 2 tasks to In Progress.\n";
            $passedTests++;
        } else {
            echo "FAIL: Bulk update failed. Updated count: $bulkUpdated (expected 2)\n";
            $failedTests++;
        }
        
        // Test bulk update with mixed valid/invalid IDs
        $totalTests++;
        $mixedBulk = $this->bulkUpdateStatus([$id4, 99999, $id7], self::STATUS_PENDING);
        if ($mixedBulk === 2 && 
            $this->tasks[$id4]->status === self::STATUS_PENDING && 
            $this->tasks[$id7]->status === self::STATUS_PENDING) {
            echo "SUCCESS: Bulk update with mixed IDs updated only valid tasks (2/3).\n";
            $passedTests++;
        } else {
            echo "FAIL: Bulk update with mixed IDs failed. Updated: $mixedBulk (expected 2)\n";
            $failedTests++;
        }
        
        // Test bulk update with invalid status (should throw exception)
        $totalTests++;
        try {
            $this->bulkUpdateStatus([$id1], 'InvalidStatus');
            echo "FAIL: Bulk update accepted invalid status.\n";
            $failedTests++;
        } catch (\InvalidArgumentException $e) {
            echo "SUCCESS: Bulk update correctly rejected invalid status.\n";
            $passedTests++;
        }

        // Import into a fresh manager instance to ensure import works and handles ID collisions
        $newManager = new TaskManager();
        // Clear newManager's storage to avoid interference
        if (file_exists('tasks.json')) {
            $backupContent = file_get_contents('tasks.json');
            unlink('tasks.json');
        }
        
        $totalTests++;
        $imported = $newManager->importTasksFromJson($exportFile);
        // Compare against the count BEFORE duplication since export happened before duplicate
        if ($imported === $taskCountBeforeDuplicate) {
            echo "SUCCESS: Imported $imported tasks into new manager instance.\n";
            $passedTests++;
        } else {
            echo "FAIL: Import count mismatch. Expected: $taskCountBeforeDuplicate, Got: $imported\n";
            $failedTests++;
        }
        
        // Test import with ID collision
        $totalTests++;
        $collisionImported = $this->importTasksFromJson($exportFile);
        if ($collisionImported > 0) {
            echo "SUCCESS: Import with ID collisions handled ($collisionImported tasks imported with new IDs).\n";
            $passedTests++;
        } else {
            echo "FAIL: Import with ID collisions failed.\n";
            $failedTests++;
        }
        
        // Test import non-existent file
        $totalTests++;
        $nonExistImport = $this->importTasksFromJson('non_existent_file.json');
        if ($nonExistImport === 0) {
            echo "SUCCESS: Import non-existent file correctly returned 0.\n";
            $passedTests++;
        } else {
            echo "FAIL: Import non-existent file should return 0.\n";
            $failedTests++;
        }
        
        // Test import corrupted JSON
        $totalTests++;
        $corruptedFile = 'corrupted_test.json';
        file_put_contents($corruptedFile, '{"tasks": [invalid json}');
        $corruptedImport = $this->importTasksFromJson($corruptedFile);
        if ($corruptedImport === 0) {
            echo "SUCCESS: Import corrupted JSON correctly returned 0.\n";
            $passedTests++;
        } else {
            echo "FAIL: Import corrupted JSON should return 0.\n";
            $failedTests++;
        }
        unlink($corruptedFile);
        
        // Test export to read-only directory (if possible - skip on failure)
        $totalTests++;
        $invalidExport = $this->exportTasksToJson('/root/invalid_path_test.json');
        if ($invalidExport === false) {
            echo "SUCCESS: Export to invalid path correctly returned false.\n";
            $passedTests++;
        } else {
            echo "INFO: Export to invalid path did not fail (may have permissions).\n";
            $passedTests++;
        }
        ...
        // Restore original tasks.json if it existed
        if (isset($backupContent)) {
            file_put_contents('tasks.json', $backupContent);
        }
        ...
        echo sprintf("%sPassed: %d%s\n", self::COLOR_GREEN, $passedTests, self::COLOR_RESET);
        echo sprintf("%sFailed: %d%s\n", ($failedTests > 0 ? self::COLOR_RED : self::COLOR_RESET), $failedTests, self::COLOR_RESET);
        $passRate = $totalTests > 0 ? round(($passedTests / $totalTests) * 100, 2) : 0;
        echo sprintf("Pass Rate: %.2f%%\n", $passRate);
        
        if ($failedTests === 0) {
            echo self::COLOR_GREEN . self::COLOR_BOLD . "🎉 ALL TESTS PASSED! 🎉" . self::COLOR_RESET . "\n";
        } else {
            echo self::COLOR_RED . self::COLOR_BOLD . "⚠️  SOME TESTS FAILED  ⚠️" . self::COLOR_RESET . "\n";
        }
        echo "==================================================\n";
    }
}
